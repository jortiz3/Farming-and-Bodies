    ...
    public string TriggerEvent(string name)
    {
        ...
        if (temp[0].Equals("Give") || temp[0].Equals("give"))
        {
            ...
            else if (temp[1].Equals("Item") || temp[1].Equals("item"))
            {
                try
                {
                    global.playerInventory.AddItem(int.Parse(temp[2]), int.Parse(temp[3]));
                }
                catch
                {
                    try
			         ...
			         temp[1].Equals("Gold") || temp[1].Equals("gold"))
            {
                try
                {
                    global.playerInventory.AddMoney(int.Parse(temp[2]));
                }
                catch
                {
            ...
            else if (temp[1].Equals("Bodies") || temp[1].Equals("bodies"))
            {
                try
                {
                    global.playerInventory.PickUpBodies(int.Parse(temp[2]));
                }
                catch
                {
		...
		else if (temp[0].Equals("NPC") || temp[0].Equals("npc"))
		{
			...
			} catch {return "Could not Parse. Command Format: npc <npc_name> <methodName> <x,y,z> or npc <npc_name> <methodName> <target_obj_name>";}
		}
