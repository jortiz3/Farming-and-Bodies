...
public class MusicManager : MonoBehaviour 
{
	...
	public TrackInfoClass[] trackList;		//Uses the class below to take in information about the track

	private float waitTime;							//Time to wait between songs
	private float timeCounter = 0;					//Used for the downtime between songs
	private int currentTrack;						//Current track number
	...
	void Update()
	{
		if (trackList == null || trackList.Length < 1)
			return;
		if(audio.isPlaying && !NPCScript.inCombat)
			return;
	...
	void PlayCombatTracks()
	{
		...
		audio.Stop();
		audio.clip = trackList[currentTrack].audioList;
		//audio.Play ();
	...
	void GoToNextTrack()	//Selects next track to play and plays it
	{
		...
		for(int i = 0; i < trackList.Length; i++)
		{
			...
			{
				audio.enabled = true;
				if(trackList[currentTrack].songType.Equals(Application.loadedLevelName))
				{
...
{
	public AudioClip audioList;				//Clips
	public string songType;					//Used to select when the song should be played 
	public Toggle musicToggle;				//Used to access the ability to turn music on and off
	public Slider musicSlider;				//Used to access the slider and determine the probability of songs to play
	[HideInInspector]
	public float probability;				//Probability that the player has selected for the music to play
}
