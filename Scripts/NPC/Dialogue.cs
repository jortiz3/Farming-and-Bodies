...
public class PlayerChoice {
	...
	public int Select() {
		...
		} catch { Debug.Log("Failed to trigger event."); }
...
public class ItemRequirement
{
	...
	public bool isSatisfied() {
		...
		if (name.Equals ("Money") || name.Equals ("money") || name.Equals ("gold") || name.Equals ("Gold")) {
			if (global.playerInventory.money < quantity)
				return false;
			return true;
		}

		if (!global.playerInventory.HasItem(name))
			return false;

		int itemqty = global.playerInventory.GetItem (name).quantity;
	...
	public void Enforce() {
		if (removeQuantityOnSelect && isSatisfied()) {
			if (name.Equals ("Money") || name.Equals ("money") || name.Equals ("gold") || name.Equals ("Gold")) {
				global.playerInventory.money -= quantity;
				return;
			}
			global.playerInventory.RemoveItem(name, quantity);
		}
	}
}
